...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot\backend'; if (Test-Path 'venv\Scripts\Activate.ps1') { .\venv\Scripts\Activate.ps1 } elseif (Test-Path '..\venv\Scripts\Activate.ps1') { ..\venv\Scripts\Activate.ps1 }; Write-Host 'Backend running: http://localhost:8000' -ForegroundColor Green; Write-Host 'Swagger UI: http://localhost:8000/docs' -ForegroundColor Green; python -m uvicorn backend.app.main:app --reload --host 127.0.0.1 --port 8000"

Start-Sleep -Seconds 3

# Start Frontend
Write-Host "[2/2] Starting Frontend..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot\frontend\frontend'; Write-Host 'Frontend running: http://localhost:8080' -ForegroundColor Green; npm run dev"
